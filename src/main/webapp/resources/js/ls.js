...
ls ={
		...
		album :()=>{
			if(!($("#albumSec").length >0)){ 
				...
				$('<h1/>').attr({id : 'alph',style :'text-align:left'}).addClass("panel-heading").html(
		 ...
		 music_list : ()=>{
				$('<div/>').attr({id: 'jt_search_music'}).addClass('container').appendTo($('#chartSec'));//슬누나 #contents -> #chartSec 바꿨어요
